...
const Task *wb_command_set_task_at(const CommandSet *set,size_t index){
    ...
    if((set->source==COMMAND_SET_USER||set->source==COMMAND_SET_PLUGIN)&&index<set->user_task_count)return &set->user_tasks[index].task;
    return NULL;
}
...
static size_t user_set_count(const CommandSetStore *store){
    size_t n=0;
    if(store)for(size_t i=0;i<store->count;i++)if(store->sets[i].source==COMMAND_SET_USER&&!store->sets[i].deleted)n++;
    return n;
}
...
int wb_command_set_create_user(CommandSetStore *store,const char *id,const char *name_zh,const char *name_en,const char *description,CommandSet **out){
    ...
    if(store->count>=WB_MAX_COMMAND_SETS||user_set_count(store)>=WB_MAX_USER_COMMAND_SETS||wb_command_sets_find(store,id))return -1;
...
int wb_command_sets_load_user(CommandSetStore *store){
    ...
    while((de=readdir(d))){
        ...
        if(store->count>=WB_MAX_COMMAND_SETS||user_set_count(store)>=WB_MAX_USER_COMMAND_SETS)break;
...
static int shell_quote_path(const char *src,char *out,size_t cap){
    if(!src||!out||cap<3)return -1;
    size_t u=0;out[u++]='\'';
    for(const char *p=src;*p;p++){
        if(*p=='\''){const char *q="'\\''";for(size_t i=0;q[i];i++){if(u+2>cap)return -1;out[u++]=q[i];}}
        else {unsigned char c=(unsigned char)*p;if(c<0x20||c==0x7f)return -1;if(u+2>cap)return -1;out[u++]=*p;}
    }
    if(u+2>cap)return -1;
    out[u++]='\'';out[u]='\0';return 0;
}

static int replace_plugin_root(WbUserTask *t,const char *plugin_root){
    if(!t||!plugin_root||!*plugin_root)return 0;
    const char needle[]="{plugin_root}";if(!strstr(t->command,needle))return 0;
    char quoted[8192];if(shell_quote_path(plugin_root,quoted,sizeof(quoted)))return -1;
    char out[sizeof(t->command)];size_t u=0;const char *p=t->command;
    while(*p){const char *m=strstr(p,needle);size_t n=m?(size_t)(m-p):strlen(p);if(u+n+1>sizeof(out))return -1;memcpy(out+u,p,n);u+=n;if(!m)break;size_t q=strlen(quoted);if(u+q+1>sizeof(out))return -1;memcpy(out+u,quoted,q);u+=q;p=m+sizeof(needle)-1;}
    out[u]='\0';if(copy_text(t->command,sizeof(t->command),out))return -1;rebind_owned_task(t);return 0;
}

int wb_command_sets_load_plugin_dir_at_root(CommandSetStore *store,const char *plugin_id,const char *directory,const char *plugin_root){
    if(!store||!valid_id(plugin_id)||!directory||!*directory)return -1;
    DIR *d=opendir(directory);if(!d)return errno==ENOENT?0:-1;struct dirent *de;
    while((de=readdir(d))){
        if(store->count>=WB_MAX_COMMAND_SETS)break;
        size_t n=strlen(de->d_name);if(n<=4||strcmp(de->d_name+n-4,".wbc"))continue;
        char set_id[64];if(n-4>=sizeof(set_id))continue;memcpy(set_id,de->d_name,n-4);set_id[n-4]='\0';if(!valid_id(set_id))continue;
        char path[4096];int pn=snprintf(path,sizeof(path),"%s/%s",directory,de->d_name);if(pn<0||(size_t)pn>=sizeof(path))continue;
        struct stat st;if(lstat(path,&st)||!S_ISREG(st.st_mode))continue;CommandSet parsed;if(parse_metadata_file(path,set_id,&parsed))continue;
        char internal[64],source[64];int in=snprintf(internal,sizeof(internal),"plugin.%s.%s",plugin_id,set_id);int sn=snprintf(source,sizeof(source),"plugin.%s",plugin_id);
        if(in<0||(size_t)in>=sizeof(internal)||sn<0||(size_t)sn>=sizeof(source)||wb_command_sets_find(store,internal)){wb_command_set_destroy(&parsed);continue;}
        if(copy_text(parsed.id,sizeof(parsed.id),internal)){wb_command_set_destroy(&parsed);continue;}parsed.source=COMMAND_SET_PLUGIN;parsed.read_only=1;
        int bad=0;for(size_t i=0;i<parsed.user_task_count;i++){
            if(copy_text(parsed.user_tasks[i].source_id,sizeof(parsed.user_tasks[i].source_id),source)||replace_plugin_root(&parsed.user_tasks[i],plugin_root)){bad=1;break;}rebind_owned_task(&parsed.user_tasks[i]);
        }
        if(bad){wb_command_set_destroy(&parsed);continue;}store->sets[store->count++]=parsed;
    }
    closedir(d);return 0;
}

int wb_command_sets_load_plugin_dir(CommandSetStore *store,const char *plugin_id,const char *directory){return wb_command_sets_load_plugin_dir_at_root(store,plugin_id,directory,NULL);}

void wb_command_sets_remove_plugins(CommandSetStore *store){
    if(!store)return;
    size_t w=0;
    for(size_t i=0;i<store->count;i++){
        if(store->sets[i].source==COMMAND_SET_PLUGIN){wb_command_set_destroy(&store->sets[i]);continue;}
        if(w!=i){store->sets[w]=store->sets[i];memset(&store->sets[i],0,sizeof(store->sets[i]));}
        w++;
    }
    store->count=w;
}
