...
#define WB_MAX_PLUGIN_COMMAND_SETS 128
#define WB_MAX_COMMAND_SETS (1 + WB_MAX_USER_COMMAND_SETS + WB_MAX_PLUGIN_COMMAND_SETS)
...
typedef struct {
    CommandSet sets[WB_MAX_COMMAND_SETS];
    size_t count;
    char root[4096];
} CommandSetStore;
...
int wb_command_sets_load_plugin_dir(CommandSetStore *store,const char *plugin_id,const char *directory);
