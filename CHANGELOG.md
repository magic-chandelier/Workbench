...
## 0.16.1-dev4 - 2026-09-22

- Fixed interactive plugin terminal ownership: Applications and interactive File Actions now receive the controlling TTY foreground process group before exec, preventing `tcsetattr()` from stopping the plugin with `SIGTTOU`.
- Added a synchronized foreground handoff so plugin code cannot race terminal ownership during startup.
- Workbench now reclaims the foreground TTY safely after plugin exit.
- Successful Application exit code 0 returns directly to Applications without an extra confirmation screen.
- Added PTY regression coverage for a plugin that enters noncanonical/no-echo mode and exits on a single `q` byte.

## 0.16.1-dev3 - 2026-09-22

- Plugin/Extension API v3 adds generic read-only Files Preview Providers while retaining API v1/v2 compatibility.
- Preview providers use bounded descriptor parsing and separate list/materialize process protocols; Core revalidates plugin state and descriptors before use.
- Workbench Files can browse plugin-provided virtual archive trees with its native keyboard/mouse UI; archive rename/move/delete/chmod remain intentionally unavailable.
- Plugin command sets can use `{plugin_root}`, expanded to a shell-quoted absolute plugin root at load time.
- Preview listing output is capped and member paths are independently rejected by Core when absolute, empty-component, dot/dotdot, or control-character paths are returned.

## 0.16.1-dev2 - 2026-09-21

- Plugin API / Extension API 升到 2，同时把宿主兼容范围定义为 API 1..2；既有 API 1 Application/Command Sets 插件继续有效。
- `plugin.wbp` 新增 `file_actions` 目录贡献；新增 `file-actions/*.wba` 通用描述格式和扩展名/目标类型过滤。
- Workbench Files 右键菜单接入插件 File Actions；Core 只处理通用元数据和选中路径，不包含 7-Zip 或其他产品特判。
- File Action 执行协议固定为 `entry --workbench-file-action <action-id> -- <absolute-path>`，路径作为独立 argv 传入，不经过 shell。
- File Action 执行前重新刷新插件并重校验 descriptor、插件状态、API 与 executable，防止旧 registry 绕过禁用/删除/修改。
- Settings 新增 `sticky_reserved_row`，默认关闭；开启后 Workbench UI 从屏幕第 2 行开始，鼠标 Y 坐标和可用高度同步偏移。
- 修复插件 registry 扫描 system root 两次的问题。
- 新增 `test_plugin_file_actions`、Files 真实右键 PTY 和 Sticky reserved row PTY 回归。
- source-only 打包门槛新增 `__pycache__` / `.pyc` 清理与拒绝检查。

## 0.16.1-dev1 - 2026-09-20

- 以 0.16.0 稳定代码树为开发基线，不直接继承 0.18 草案的未完成 Runtime Provider。
- 完成插件 Enable/Disable：扫描识别 `.workbench-disabled`，Disabled 插件不进入 Applications、不加载 Plugin Command Sets，启动前再次校验禁用状态。
- `wb_plugin_set_enabled()` 改为 registry-root 校验 + fd-relative/no-follow marker 操作，拒绝伪造插件路径。
- Plugin Manager 增加键盘 `e` 与鼠标按钮的 Enable/Disable 操作，并显示 `Disabled` 状态。
- 新增最小 Extension Registry，分离 `WbExtensionActionDescriptor` 与 `WbRegisteredExtensionAction`，避免 0.18 草案重复类型定义；暂不引入产品特定逻辑或空壳 Runtime Provider。
- 版本常量集中到 `include/version.h`。
- 新增 `test_plugin_enable_disable` 与 `test_extensions_core`，并把新目录纳入 source-only 打包。

## 0.16.0 - 2026-09-16

- 在完整 v0.15.x 基线上增量加入 **Plugin System v0.1.0**；Workbench Files、Locations、第一方 Terminal、Command Sets、Linux Generic 374 Actions、CentOS 483 Actions、双输入/虚拟键盘和既有设置/安全行为保持原样，并继续由原有回归测试覆盖。
- 新增双层插件目录：`/usr/local/share/workbench/plugins/`（System）与 `~/.local/share/workbench/plugins/`（User）；一级目录 + 合法 `plugin.wbp` 即为插件，不维护安装数据库，手动删除目录即可卸载。
- 同 ID 有效 User 插件覆盖 System 插件；删除 User 版本后 System 版本自动恢复。无效/不兼容 User 候选不会毒死有效 System 插件。
- 新增动态 **Applications** 首页入口：只有存在带 `entry` 的 Active 插件时才显示；插件作为独立子进程/进程组运行，不通过 `dlopen()` 把第三方插件代码载入 Workbench Core。
- 新增 `Settings → Plugins`：支持 Refresh、Properties、键盘与 Mouse-only 操作和确认式 Uninstall；用户插件直接卸载，系统插件提供 root/`sudo wb --plugin-remove-system <id>` 通用入口。
- 插件卸载采用原子 `.wb-removing-*` 摘除 + fd-relative/no-follow 递归清理；路径遍历、符号链接逃逸、删除插件根目录等危险情况被拒绝。
- 新增 Plugin Command Sets：插件目录可提供 `.wbc`，在原有浏览器中显示为 `[Plugin] [Read-only]`，首页 EXECUTE 与 Terminal INSERT 语义保持一致。
- 新增 Plugin API 1 manifest 校验、运行前二次校验、崩溃隔离、稳定 registry 排序，以及坏插件隔离；Core 不包含 Docker/7Z/Nginx 等单独适配分支。
- `--self-test` 新增真实 `plugins_guard=PASS`；新增 plugin registry/runtime/Command Sets、系统卸载、Core boundary、Applications、Plugin Manager Mouse-only 和 Terminal Plugin INSERT 回归。
- 新增 `docs/PLUGIN_FORMAT.md` 与 `examples/plugins/hello/`；source-only 发布继续不携带编译后的 `wb`。
