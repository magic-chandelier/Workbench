...
VERSION := 0.16.1-dev4
TARGET := wb
SRC := main.c modules/linux-core/linux_core.c modules/centos/centos.c modules/files-manager/files_manager.c modules/terminal/terminal.c modules/command-sets/command_sets.c modules/plugins/plugins.c modules/extensions/extensions.c
HEADERS := workbench.h modules/linux-core/linux_core.h modules/centos/centos.h modules/files-manager/files_manager.h modules/terminal/terminal.h modules/command-sets/command_sets.h modules/plugins/plugins.h include/version.h modules/extensions/extensions.h
...
TEST_SCRIPTS := \
	...
	tests/test_plugins_core.sh \
	tests/test_plugin_enable_disable.sh \
	tests/test_extensions_core.sh \
	tests/test_plugin_file_actions.sh \
	tests/test_plugin_previews.sh \
	tests/test_plugins_runtime.sh \
	tests/test_plugin_command_sets.sh \
	tests/test_plugins_selftest.sh \
	tests/test_system_plugin_remove.sh \
	tests/test_plugin_core_boundary.sh \
...
test: $(TARGET)
	...
	python3 tests/smoke_files_plugin_action.py
	python3 tests/smoke_files_plugin_preview.py
	...
	python3 tests/smoke_sticky_row.py
	...
	python3 tests/smoke_plugins.py
	python3 tests/smoke_plugin_tty_handoff.py
	python3 tests/smoke_plugin_toggle.py
	python3 tests/smoke_plugins_mouse.py
	python3 tests/smoke_terminal_plugin_command_sets.py
...
package-only:
	...
	cp -R modules tests docs examples include dist/workbench-v$(VERSION)/
	find dist/workbench-v$(VERSION) -type d -name __pycache__ -prune -exec rm -rf {} +
	find dist/workbench-v$(VERSION) -type f -name '*.pyc' -delete
