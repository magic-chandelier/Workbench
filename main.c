...
static int g_sticky_reserved_row = 0;
...
static WbPluginRegistry g_plugins;
static WbExtensionRegistry g_extensions;
static int g_plugins_initialized = 0;
static char g_plugin_system_root[WB_PLUGIN_PATH_MAX];
static char g_plugin_user_root[WB_PLUGIN_PATH_MAX];
...
static void plugin_manager_menu(AppConfig *c);
static void applications_menu(AppConfig *c);
static int refresh_plugins(void);
...
static const char *task_source_name(const Task*t,Language l){
    ...
    if(t->source_id&&!strncmp(t->source_id,"plugin.",7))return tr(l,"插件","Plugin");
...
static const char *task_source_marker(const Task*t){
    ...
    if(t->source_id&&!strncmp(t->source_id,"plugin.",7))return "Plugin";
    return "";
}
...
static int ui_screen_y(int logical_y){return logical_y+g_sticky_reserved_row;}
static void clear_screen(void) { printf("\033[2J\033[%d;1H",1+g_sticky_reserved_row); }
static int terminal_width(void) { struct winsize w; return ioctl(STDOUT_FILENO,TIOCGWINSZ,&w)==0&&w.ws_col? w.ws_col:80; }
static int terminal_height(void) { struct winsize w;int h=ioctl(STDOUT_FILENO,TIOCGWINSZ,&w)==0&&w.ws_row?w.ws_row:24;h-=g_sticky_reserved_row;return h>0?h:1; }
...
static UiEvent ui_read_event_ready(void) {
    ...
    if(b=='<'){
        ...
        int mb=-1,x=0,y=0;
        if(sscanf(buf,"%d;%d;%d",&mb,&x,&y)==3){ev.x=x;ev.y=y-g_sticky_reserved_row;ev.button=mb&3;if(mb&64){ev.kind=(mb&1)?UI_EVENT_WHEEL_DOWN:UI_EVENT_WHEEL_UP;return ev;}ev.kind=(ch=='m')?UI_EVENT_MOUSE_RELEASE:UI_EVENT_MOUSE_PRESS;return ev;}
    }
...
static void config_defaults(AppConfig*c){c->language=LANG_ZH;c->confirm_normal=0;c->confirm_sensitive=0;c->confirm_privileged=1;c->linux_profile=PROFILE_GENERIC;c->virtual_keyboard=0;c->shortcut_hints=1;c->sticky_reserved_row=0;c->terminal_log_lines=1000;c->terminal_load_lines=50;c->terminal_clear_on_open=0;}
static void apply_ui_preferences(const AppConfig*c){g_virtual_keyboard_enabled=c->virtual_keyboard;g_shortcut_hints_enabled=c->shortcut_hints;g_sticky_reserved_row=c->sticky_reserved_row?1:0;}
static void load_config(AppConfig*c){char p[PATH_MAX],l[128];config_defaults(c);if(config_path(p,sizeof(p))){apply_ui_preferences(c);return;}FILE*f=fopen(p,"r");if(!f){apply_ui_preferences(c);return;}while(fgets(l,sizeof(l),f)){if(!strncmp(l,"language=en",11))c->language=LANG_EN;else if(!strncmp(l,"language=zh",11))c->language=LANG_ZH;else if(!strncmp(l,"confirm_normal=1",16))c->confirm_normal=1;else if(!strncmp(l,"confirm_normal=0",16))c->confirm_normal=0;else if(!strncmp(l,"confirm_sensitive=1",19))c->confirm_sensitive=1;else if(!strncmp(l,"confirm_sensitive=0",19))c->confirm_sensitive=0;else if(!strncmp(l,"confirm_privileged=1",20))c->confirm_privileged=1;else if(!strncmp(l,"confirm_privileged=0",20))c->confirm_privileged=0;else if(!strncmp(l,"linux_profile=centos",20))c->linux_profile=PROFILE_CENTOS;else if(!strncmp(l,"linux_profile=generic",21))c->linux_profile=PROFILE_GENERIC;else if(!strncmp(l,"virtual_keyboard=1",18))c->virtual_keyboard=1;else if(!strncmp(l,"virtual_keyboard=0",18))c->virtual_keyboard=0;else if(!strncmp(l,"shortcut_hints=1",16))c->shortcut_hints=1;else if(!strncmp(l,"shortcut_hints=0",16))c->shortcut_hints=0;else if(!strncmp(l,"sticky_reserved_row=1",21))c->sticky_reserved_row=1;else if(!strncmp(l,"sticky_reserved_row=0",21))c->sticky_reserved_row=0;else if(!strncmp(l,"terminal_log_lines=",19)){int v=atoi(l+19);if(v>=100&&v<=10000)c->terminal_log_lines=v;}else if(!strncmp(l,"terminal_load_lines=",20)){int v=atoi(l+20);if(v>=10&&v<=500)c->terminal_load_lines=v;}else if(!strncmp(l,"terminal_clear_on_open=1",24))c->terminal_clear_on_open=1;else if(!strncmp(l,"terminal_clear_on_open=0",24))c->terminal_clear_on_open=0;}fclose(f);apply_ui_preferences(c);}
static const char *linux_profile_key(LinuxProfile p){return p==PROFILE_CENTOS?"centos":"generic";}
static int save_config(const AppConfig*c){char p[PATH_MAX];apply_ui_preferences(c);if(ensure_config_dir()||config_path(p,sizeof(p)))return-1;FILE*f=fopen(p,"w");if(!f)return-1;fprintf(f,"language=%s\nconfirm_normal=%d\nconfirm_sensitive=%d\nconfirm_privileged=%d\nlinux_profile=%s\nvirtual_keyboard=%d\nshortcut_hints=%d\nsticky_reserved_row=%d\nterminal_log_lines=%d\nterminal_load_lines=%d\nterminal_clear_on_open=%d\n",c->language==LANG_ZH?"zh":"en",c->confirm_normal,c->confirm_sensitive,c->confirm_privileged,linux_profile_key(c->linux_profile),c->virtual_keyboard,c->shortcut_hints,c->sticky_reserved_row,c->terminal_log_lines,c->terminal_load_lines,c->terminal_clear_on_open);return fclose(f);}
...
static void ui_wait_return_inline(Language l){
    int y=terminal_height();if(y<1)y=1;printf("\033[%d;1H",ui_screen_y(y));ui_hits_reset();int x=1;ui_print_button(1,y,&x,tr(l,"返回","Return"));if(g_shortcut_hints_enabled)printf(" %s",tr(l,"Enter/q 也可返回","Enter/q also returns"));fflush(stdout);
...
static const char *plugin_name(const WbPlugin *p,Language l){return l==LANG_ZH?p->name_zh:p->name_en;}
static const char *plugin_desc(const WbPlugin *p,Language l){return l==LANG_ZH?p->description_zh:p->description_en;}
static const char *plugin_status_name(WbPluginStatus st,Language l){
    if(st==WB_PLUGIN_ACTIVE)return tr(l,"可用","Active");
    if(st==WB_PLUGIN_INVALID)return tr(l,"无效","Invalid");
    if(st==WB_PLUGIN_INCOMPATIBLE)return tr(l,"不兼容","Incompatible");
    if(st==WB_PLUGIN_DISABLED)return tr(l,"已禁用","Disabled");
    return tr(l,"被覆盖","Shadowed");
}
static size_t plugin_application_count(void){
    size_t n=0;if(!g_plugins_initialized)return 0;
    for(size_t i=0;i<g_plugins.count;i++)if(g_plugins.items[i].status==WB_PLUGIN_ACTIVE&&g_plugins.items[i].entry[0])n++;
    return n;
}
static const WbPlugin *plugin_application_at(size_t index){
    if(!g_plugins_initialized)return NULL;
    for(size_t i=0;i<g_plugins.count;i++)if(g_plugins.items[i].status==WB_PLUGIN_ACTIVE&&g_plugins.items[i].entry[0]){if(index==0)return &g_plugins.items[i];index--;}
    return NULL;
}
static int refresh_plugins(void){
    if(!g_plugins_initialized)return -1;
    if(wb_plugin_registry_scan(&g_plugins,g_plugin_system_root,g_plugin_user_root))return -1;
    if(wb_extension_registry_load_plugin_actions(&g_extensions,&g_plugins))return -1;
    if(g_command_sets_initialized){
        wb_command_sets_remove_plugins(&g_command_sets);
        for(size_t i=0;i<g_plugins.count;i++){
            const WbPlugin *p=&g_plugins.items[i];
            if(p->status!=WB_PLUGIN_ACTIVE)continue;
            char dir[WB_PLUGIN_PATH_MAX];int n=snprintf(dir,sizeof(dir),"%s/%s",p->root,p->command_sets);
            if(n<0||(size_t)n>=sizeof(dir))continue;
            (void)wb_command_sets_load_plugin_dir_at_root(&g_command_sets,p->id,dir,p->root);
        }
    }
    return 0;
}
static void plugin_properties(AppConfig *c,const WbPlugin *p){
    Language l=c->language;
    for(;;){
        int w=terminal_width();if(w>110)w=110;clear_screen();ui_hits_reset();printf("\033[?25lWorkbench %s | %s\n",WB_VERSION,tr(l,"插件属性","Plugin Properties"));print_rule(w);
        printf("%s: %s\n",tr(l,"名称","Name"),plugin_name(p,l));
        printf("%s: %s\n",tr(l,"ID","ID"),p->id);
        printf("%s: %s\n",tr(l,"版本","Version"),p->version);
        printf("%s: %s\n",tr(l,"来源","Source"),p->source==WB_PLUGIN_USER?tr(l,"用户","User"):tr(l,"系统","System"));
        printf("%s: %s\n",tr(l,"状态","Status"),plugin_status_name(p->status,l));
        printf("%s: ",tr(l,"路径","Path"));print_truncated(p->root,w-8);putchar('\n');
        if(p->error[0]){printf("%s: ",tr(l,"错误","Error"));print_truncated(p->error,w-9);putchar('\n');}
        putchar('\n');int x=2;ui_print_button(1,12,&x,tr(l,"返回","Back"));putchar('\n');fflush(stdout);
        UiEvent ev=ui_read_event();if(ev.kind==UI_EVENT_EOF)return;if(ev.kind==UI_EVENT_KEY&&(ev.key=='\r'||ev.key=='\n'||ev.key==27||ev.key=='q'||ev.key=='Q'))return;if(ev.kind==UI_EVENT_MOUSE_PRESS&&ev.button==0&&ui_hit_test(&ev)==1)return;
    }
}

static int set_plugin_enabled_interactive(AppConfig *c,const WbPlugin *p){
    if(!p||(p->status!=WB_PLUGIN_ACTIVE&&p->status!=WB_PLUGIN_DISABLED))return 0;
    Language l=c->language;int enable=p->status==WB_PLUGIN_DISABLED;char err[256]={0};
    int rc=wb_plugin_set_enabled(&g_plugins,p,enable,err,sizeof(err));
    refresh_plugins();
    if(rc)show_message(l,err[0]?err:"插件状态修改失败。",err[0]?err:"Plugin state change failed.");
    return rc==0;
}

static int self_exe_path(char *out,size_t cap){ssize_t n=readlink("/proc/self/exe",out,cap-1);if(n<=0||(size_t)n>=cap-1)return -1;out[n]='\0';return 0;}
static int uninstall_plugin_interactive(AppConfig *c,const WbPlugin *p){
    Language l=c->language;char q[512];snprintf(q,sizeof(q),l==LANG_ZH?"确认卸载插件 %s？只删除插件程序目录。":"Uninstall plugin %s? Only the plugin program directory will be removed.",plugin_name(p,l));
    if(!confirm_line(l,q,q))return 0;
    char err[256]={0};int rc=-1;
    if(p->source==WB_PLUGIN_USER||geteuid()==0)rc=wb_plugin_uninstall(&g_plugins,p,err,sizeof(err));
    else{
        char self[PATH_MAX];if(self_exe_path(self,sizeof(self))){snprintf(err,sizeof(err),"cannot resolve Workbench executable");rc=-1;}
        else{
            restore_terminal();pid_t ch=fork();int st=0;
            if(ch==0){execlp("sudo","sudo",self,"--plugin-remove-system",p->id,(char*)NULL);_exit(127);}
            if(ch>0){while(waitpid(ch,&st,0)<0&&errno==EINTR){}rc=(WIFEXITED(st)&&WEXITSTATUS(st)==0)?0:-1;}
            if(enable_raw_mode())exit(1);
            if(rc)snprintf(err,sizeof(err),"sudo uninstall failed");
        }
    }
    refresh_plugins();
    if(rc)show_message(l,err[0]?err:"插件卸载失败。",err[0]?err:"Plugin uninstall failed.");
    return rc==0;
}
static void plugin_manager_menu(AppConfig *c){
    size_t sel=0;
    for(;;){refresh_plugins();if(g_plugins.count&&sel>=g_plugins.count)sel=g_plugins.count-1;Language l=c->language;int w=terminal_width();if(w>116)w=116;clear_screen();ui_hits_reset();printf("\033[?25lWorkbench %s | %s\n",WB_VERSION,tr(l,"插件","Plugins"));print_rule(w);ui_hint_line(l,"键盘：↑/↓ 选择，Enter/p 属性，e 启用/禁用，d 卸载，r 刷新，Esc/q 返回。","Keyboard: Up/Down select, Enter/p properties, e enable/disable, d uninstall, r refresh, Esc/q back.");int x=1;ui_print_button(10,4,&x,tr(l,"刷新","Refresh"));ui_print_button(11,4,&x,tr(l,"属性","Properties"));ui_print_button(12,4,&x,tr(l,"卸载","Uninstall"));ui_print_button(13,4,&x,tr(l,"返回","Back"));const WbPlugin *sp=g_plugins.count?&g_plugins.items[sel]:NULL;ui_print_button(14,4,&x,(sp&&sp->status==WB_PLUGIN_DISABLED)?tr(l,"启用","Enable"):tr(l,"禁用","Disable"));putchar('\n');print_rule(w);
        if(!g_plugins.count)printf("  %s\n",tr(l,"未发现插件。","No plugins found."));
        for(size_t i=0;i<g_plugins.count;i++){const WbPlugin*p=&g_plugins.items[i];int y=6+(int)i;ui_hit_add(100+(int)i,1,y,w,y);if(i==sel)printf("\033[7m > ");else printf("   ");print_cell(plugin_name(p,l),28);print_cell(p->source==WB_PLUGIN_USER?tr(l,"用户","User"):tr(l,"系统","System"),10);print_cell(plugin_status_name(p->status,l),16);print_truncated(p->version,w-60);if(i==sel)printf("\033[0m");putchar('\n');}
        fflush(stdout);UiEvent ev=ui_read_event();if(ev.kind==UI_EVENT_EOF)return;int action=0;
        if(ev.kind==UI_EVENT_KEY){int k=ev.key;if(k==27||k=='q'||k=='Q')return;if(k==KEY_UP||k=='k'||k=='K'){if(g_plugins.count)sel=sel?sel-1:g_plugins.count-1;}else if(k==KEY_DOWN||k=='j'||k=='J'){if(g_plugins.count)sel=(sel+1)%g_plugins.count;}else if(k=='r'||k=='R')action=10;else if(k=='p'||k=='P'||k=='\r'||k=='\n')action=11;else if(k=='d'||k=='D')action=12;else if(k=='e'||k=='E')action=14;}
        else if(ev.kind==UI_EVENT_WHEEL_UP){if(g_plugins.count)sel=sel?sel-1:g_plugins.count-1;}else if(ev.kind==UI_EVENT_WHEEL_DOWN){if(g_plugins.count)sel=(sel+1)%g_plugins.count;}
        else if(ev.kind==UI_EVENT_MOUSE_PRESS&&ev.button==0){int hit=ui_hit_test(&ev);if(hit>=100&&hit<100+(int)g_plugins.count){sel=(size_t)(hit-100);action=11;}else action=hit;}
        if(action==13)return;
        if(action==10)continue;
        if(action==11&&g_plugins.count){plugin_properties(c,&g_plugins.items[sel]);continue;}
        if(action==12&&g_plugins.count){WbPlugin chosen=g_plugins.items[sel];uninstall_plugin_interactive(c,&chosen);continue;}
        if(action==14&&g_plugins.count){WbPlugin chosen=g_plugins.items[sel];set_plugin_enabled_interactive(c,&chosen);continue;}
    }
}
static void applications_menu(AppConfig *c){
    size_t sel=0;
    for(;;){refresh_plugins();size_t count=plugin_application_count();if(!count)return;if(sel>=count)sel=count-1;Language l=c->language;int w=terminal_width();if(w>112)w=112;clear_screen();ui_hits_reset();printf("\033[?25lWorkbench %s | %s\n",WB_VERSION,tr(l,"应用","Applications"));print_rule(w);ui_hint_line(l,"键盘：↑/↓ 选择，Enter 启动，p 属性，Esc/q 返回。鼠标可点击。","Keyboard: Up/Down select, Enter launch, p properties, Esc/q back. Mouse supported.");putchar('\n');
        for(size_t i=0;i<count;i++){const WbPlugin*p=plugin_application_at(i);int y=5+(int)i;ui_hit_add(100+(int)i,1,y,w,y);if(i==sel)printf("\033[7m > ");else printf("   ");print_cell(plugin_name(p,l),30);print_truncated(plugin_desc(p,l),w-35);if(i==sel)printf("\033[0m");putchar('\n');}fflush(stdout);
        UiEvent ev=ui_read_event();if(ev.kind==UI_EVENT_EOF)return;int launch=0,props=0;
        if(ev.kind==UI_EVENT_KEY){int k=ev.key;if(k==27||k=='q'||k=='Q')return;if(k==KEY_UP||k=='k'||k=='K')sel=sel?sel-1:count-1;else if(k==KEY_DOWN||k=='j'||k=='J')sel=(sel+1)%count;else if(k=='p'||k=='P')props=1;else if(k=='\r'||k=='\n')launch=1;}
        else if(ev.kind==UI_EVENT_WHEEL_UP)sel=sel?sel-1:count-1;else if(ev.kind==UI_EVENT_WHEEL_DOWN)sel=(sel+1)%count;else if(ev.kind==UI_EVENT_MOUSE_PRESS&&ev.button==0){int hit=ui_hit_test(&ev);if(hit>=100&&hit<100+(int)count){sel=(size_t)(hit-100);launch=1;}}
        const WbPlugin *p=plugin_application_at(sel);if(!p)continue;if(props){plugin_properties(c,p);continue;}if(launch){WbPlugin chosen=*p;restore_terminal();clear_screen();fflush(stdout);WbPluginRunResult rr={0};char err[256]={0};int rc=wb_plugin_launch(&chosen,&rr,err,sizeof(err));if(enable_raw_mode())exit(1);if(!rc&&rr.exited&&rr.exit_code==0&&!rr.signaled)continue;char msg[384];if(rc)snprintf(msg,sizeof(msg),"%s",err[0]?err:tr(l,"插件启动失败。","Plugin launch failed."));else if(rr.signaled)snprintf(msg,sizeof(msg),l==LANG_ZH?"插件已结束（信号 %d）。":"Plugin finished (signal %d).",rr.term_signal);else snprintf(msg,sizeof(msg),l==LANG_ZH?"插件已结束（退出码 %d）。":"Plugin finished (exit code %d).",rr.exit_code);show_message(l,msg,msg);}
    }
}
...
static void settings_menu(AppConfig*c){
    ...
    for(;;){
        ...
        int has_plugins=g_plugins_initialized&&g_plugins.count>0;size_t total=has_plugins?12:11;
        const char*z[] ={ "界面语言","Linux 版本","执行确认策略","虚拟键盘","快捷键提示","终端日志行数","终端上滑加载","重新打开终端时清屏","终端登录自启动","粘滞预留行","插件","返回主菜单" };
        const char*e[] ={ "Interface language","Linux version","Run confirmation policy","Virtual keyboard","Shortcut hints","Terminal log lines","Terminal history load","Clear terminal on reopen","Terminal login autostart","Sticky reserved row","Plugins","Back to main" };
        char pol[32],profile[64],logv[32],loadv[32],plugv[32];
        snprintf(pol,sizeof(pol),l==LANG_ZH?"%d/3 询问":"%d/3 ask",c->confirm_normal+c->confirm_sensitive+c->confirm_privileged);snprintf(profile,sizeof(profile),"%s",linux_profile_label(c->linux_profile,l));snprintf(logv,sizeof(logv),"%d",c->terminal_log_lines);snprintf(loadv,sizeof(loadv),"%d",c->terminal_load_lines);snprintf(plugv,sizeof(plugv),"%zu",g_plugins.count);
        const char*v[] ={ l==LANG_ZH?"简体中文":"English",profile,pol,c->virtual_keyboard?tr(l,"已启用","Enabled"):tr(l,"已关闭","Disabled"),c->shortcut_hints?tr(l,"已启用","Enabled"):tr(l,"已关闭","Disabled"),logv,loadv,c->terminal_clear_on_open?tr(l,"已启用","Enabled"):tr(l,"已关闭","Disabled"),auto_on?tr(l,"已启用","Enabled"):tr(l,"未启用","Disabled"),c->sticky_reserved_row?tr(l,"已启用","Enabled"):tr(l,"已关闭","Disabled"),plugv,"" };
        if(!has_plugins&&sel>=total)sel=total-1;
        ...
        for(size_t i=0;i<total;i++){
            size_t src=(!has_plugins&&i==10)?11:i;ui_hit_add(100+(int)i,1,5+(int)i,82,5+(int)i);if(i==sel)printf("\033[7m > ");else printf("   ");print_padded(l==LANG_ZH?z[src]:e[src],28);print_padded(v[src],24);if(i==sel)printf("\033[0m");putchar('\n');
        }
        putchar('\n');print_rule(82);
...
static size_t desktop_total_items(void){return plugin_application_count()?6u:5u;}
static int desktop_kind(size_t index){
    int apps=plugin_application_count()>0;
    if(!apps)return (int)index; /* 0 Files,1 Terminal,2 Command Sets,3 Settings,4 Exit */
    if(index<2)return (int)index;
    if(index==2)return 5;       /* Applications */
    return (int)index-1;       /* Command Sets=2, Settings=3, Exit=4 */
}
static void draw_desktop(size_t sel, Language l) {
    int w=terminal_width();if(w>1)w--;if(w>100)w=100;size_t total=desktop_total_items();
    ...
    printf("%s\n",tr(l,"Workbench Files、内嵌终端和指令集保留原有功能；安装应用插件后会动态出现 Applications。","Workbench Files, the embedded Terminal, and Command Sets retain their existing behavior; Applications appears dynamically when app plugins are installed."));
    print_rule(w);
    for(size_t i=0;i<total;i++){
        const char *title,*desc;char descbuf[192];int kind=desktop_kind(i);
        if(kind==0){title="Workbench Files";desc=tr(l,"本地文件浏览、查看、复制、移动、重命名、删除与权限管理","Local browse, view, copy, move, rename, delete and permissions");}
        else if(kind==1){title=tr(l,"终端","Terminal");desc=tr(l,"Workbench 内嵌 PTY Shell、短期日志和指令集插入","Embedded PTY shell, short-lived history and Command Set insertion");}
        else if(kind==5){title=tr(l,"应用","Applications");snprintf(descbuf,sizeof(descbuf),l==LANG_ZH?"已安装 %zu 个应用插件":"%zu application plugins installed",plugin_application_count());desc=descbuf;}
        else if(kind==2){title=tr(l,"指令集","Command Sets");snprintf(descbuf,sizeof(descbuf),l==LANG_ZH?"Linux 系统预制 + 用户自定义 + 插件只读 · %zu 个系统操作":"Linux system preset + user custom + plugin read-only · %zu system actions",TASK_COUNT);desc=descbuf;}
        else if(kind==3){title=tr(l,"设置","Settings");desc=tr(l,"Linux 版本、语言、双输入、终端日志、插件与执行确认","Linux profile, language, dual input, terminal history, plugins and confirmations");}
    ...
    putchar('\n');print_rule(w);printf("%s: %zu   %s: %zu   %s: %zu   %s: %zu\n",tr(l,"应用","Apps"),3+plugin_application_count(),tr(l,"指令集","Command sets"),g_command_sets_initialized?g_command_sets.count:1,tr(l,"系统模块","System modules"),module_count(),tr(l,"系统功能","System actions"),total_module_actions());fflush(stdout);
}
...
static int selftest_plugins_guard(void){
    char base[]="/tmp/wb-plugin-selftest-XXXXXX";if(!mkdtemp(base))return 0;
    char sys[PATH_MAX],usr[PATH_MAX],dir[PATH_MAX],manifest[PATH_MAX];
    int ok=0;WbPluginRegistry r;wb_plugin_registry_init(&r);
    if(snprintf(sys,sizeof(sys),"%s/system",base)>=(int)sizeof(sys)||snprintf(usr,sizeof(usr),"%s/user",base)>=(int)sizeof(usr))goto done;
    if(mkdir(sys,0700)||mkdir(usr,0700))goto done;
    if(snprintf(dir,sizeof(dir),"%s/demo",usr)>=(int)sizeof(dir)||mkdir(dir,0700))goto done;
    if(snprintf(manifest,sizeof(manifest),"%s/plugin.wbp",dir)>=(int)sizeof(manifest))goto done;
    int fd=open(manifest,O_WRONLY|O_CREAT|O_TRUNC,0600);if(fd<0)goto done;
    static const char text[]="WORKBENCH_PLUGIN=1\nformat=1\nid=demo\nversion=1\napi_min=1\napi_max=1\nname_zh=Demo\nname_en=Demo\ndescription_zh=x\ndescription_en=x\n";
    ssize_t wn=write(fd,text,sizeof(text)-1);close(fd);if(wn!=(ssize_t)(sizeof(text)-1))goto done;
    if(wb_plugin_registry_scan(&r,sys,usr))goto done;
    const WbPlugin *plug=wb_plugin_registry_find_active(&r,"demo");
    ok=plug&&plug->source==WB_PLUGIN_USER&&wb_plugin_registry_active_count(&r)==1&&wb_plugins_valid_id("demo.ok")&&!wb_plugins_valid_id("../bad");
    if(ok){char err[128];if(wb_plugin_uninstall(&r,plug,err,sizeof(err)))ok=0;}
done:
    (void)unlink(manifest);(void)rmdir(dir);(void)rmdir(usr);(void)rmdir(sys);(void)rmdir(base);wb_plugin_registry_clear(&r);return ok;
}
...
static int self_test_catalogue(void){
    ...
    printf("SELFTEST PASS profiles=2 generic_actions=%zu centos_actions=%zu centos_source=%zu overrides=%zu max_args=%d generic_system=%zu generic_files=%zu generic_text=%zu generic_process=%zu generic_network=%zu generic_storage=%zu generic_permission=%zu generic_archive=%zu generic_user=%zu centos_packages=%zu centos_services=%zu centos_firewall=%zu centos_selinux=%zu generic_syntax=%zu centos_syntax=%zu destructive_path_guard=%s files_guard=%s locations_guard=%s dual_input_guard=%s terminal_guard=%s command_sets_guard=%s plugins_guard=%s search_checked=%d\n",
           ...
           centos.counts[CAT_PACKAGE],centos.counts[CAT_SERVICE],centos.counts[CAT_FIREWALL],centos.counts[CAT_SELINUX],generic.syntax_checked,centos.syntax_checked,path_guard_ok?"PASS":"FAIL",files_guard_ok?"PASS":"FAIL",locations_guard_ok?"PASS":"FAIL",dual_input_guard_ok?"PASS":"FAIL",terminal_guard_ok?"PASS":"FAIL",command_sets_guard_ok?"PASS":"FAIL",plugins_guard_ok?"PASS":"FAIL",search_checks);
    return 0;
}

int main(int argc, char **argv) {
    if(argc==2&&strcmp(argv[1],"--version")==0){printf("Workbench %s\nPlugin System 0.2.0 (API %d, compatible %d-%d)\nExtension API %d\n",WB_VERSION,WB_PLUGIN_API_VERSION,WB_PLUGIN_API_MIN_SUPPORTED,WB_PLUGIN_API_VERSION,WB_EXTENSION_API_VERSION);return 0;}
    if(argc==3&&strcmp(argv[1],"--plugin-remove-system")==0){
        if(geteuid()!=0){fprintf(stderr,"system plugin removal requires root\n");return 3;}
        const char *root="/usr/local/share/workbench/plugins";
        const char *allow=getenv("WB_ALLOW_TEST_ROOTS"),*testroot=getenv("WB_PLUGIN_SYSTEM_ROOT");
        if(allow&&!strcmp(allow,"1")&&testroot&&*testroot)root=testroot;
        char err[256]={0};if(wb_plugin_remove_system_by_id(root,argv[2],err,sizeof(err))){fprintf(stderr,"%s\n",err[0]?err:"plugin removal failed");return 1;}return 0;
    }
    ...
    wb_plugin_registry_init(&g_plugins);wb_extension_registry_init(&g_extensions);if(wb_plugins_default_roots(g_plugin_system_root,sizeof(g_plugin_system_root),g_plugin_user_root,sizeof(g_plugin_user_root)))return 1;g_plugins_initialized=1;if(refresh_plugins())return 1;
    if(enable_raw_mode())return 1;
    size_t sel=0;
    for(;;){
        (void)refresh_plugins();size_t total=desktop_total_items();if(sel>=total)sel=0;draw_desktop(sel,c.language);int k=read_key();if(k==-1)break;
        ...
        else if(k==KEY_MOUSE&&!mouse_release&&mouse_button==0){int ro=mouse_y-6;if(ro>=0&&(size_t)ro<total){sel=(size_t)ro;int kind=desktop_kind(sel);if(kind==0)files_manager_menu(&c);else if(kind==1)terminal_menu(&c);else if(kind==5)applications_menu(&c);else if(kind==2)command_sets_menu(&c,COMMAND_MODE_EXECUTE,NULL,0);else if(kind==3)settings_menu(&c);else break;}}
        else if(k=='\r'||k=='\n'){int kind=desktop_kind(sel);if(kind==0)files_manager_menu(&c);else if(kind==1)terminal_menu(&c);else if(kind==5)applications_menu(&c);else if(kind==2)command_sets_menu(&c,COMMAND_MODE_EXECUTE,NULL,0);else if(kind==3)settings_menu(&c);else break;}
    }
    ...
    if(g_plugins_initialized){wb_plugin_registry_clear(&g_plugins);g_plugins_initialized=0;}
